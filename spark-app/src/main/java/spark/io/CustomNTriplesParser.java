...
public class CustomNTriplesParser extends AbstractRDFParser {
	...
	ObjectBigArrayBigList<Tuple2<String,Tuple2<String,String>>> result = 
			new ObjectBigArrayBigList<Tuple2<String,Tuple2<String,String>>>(50000);
	
	String subject;
	String predicate;
	String object;
	StringBuilder datatypeUri = new StringBuilder();
	...
	public ObjectBigArrayBigList<Tuple2<String, Tuple2<String,String>>> getResult(){
		return result;
	}
	/**
	...
	protected int parseSubject(int c, StringBuilder sb)
		throws IOException, RDFParseException
	{
	...
	protected int parsePredicate(int c, StringBuilder sb)
		throws IOException, RDFParseException
	{
		//StringBuilder sb = new StringBuilder(100);
	...
	protected int parseObject(int c, StringBuilder sb)
		throws IOException, RDFParseException
	{
	...
	protected int parseUriRef(int c, StringBuilder sb)
		throws IOException, RDFParseException
	{
		if (c != '<') {
	...
	protected int parseNodeID(int c, StringBuilder sb)
		throws IOException, RDFParseException
	{
		if (c != '_') {
	...
	private int parseLiteral(int c, StringBuilder sb, StringBuilder lang, StringBuilder datatype)
		throws IOException, RDFParseException
	{
		if (c != '"') {
		...
		if (c == '@') {
			...
			while (c != -1 && c != '.' && c != '^' && c != ' ' && c != '\t') {
				this.languageTag.append(Character.toChars(c));
				c = readCodePoint();
			}
		}
		else if (c == '^') {
	 ...
	 */
	/*private String getLanguageTagBuffer() {
		languageTag = "";
	 ...
	 */
	/*private String getDatatypeUriBuffer() {
		datatypeUri = "";
