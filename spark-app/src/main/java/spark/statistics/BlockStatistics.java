...
public class BlockStatistics {
	...
	/**
	 * @param blockSizesRDD (token, blockSize)
	 * @return optimal block size
	 */
	public static int getOptimalBlockSize(JavaPairRDD<String, Integer> blockSizesRDD) {
		...
		//(N,1)
		blockSizesRDD.mapToPair(new PairFunction<Tuple2<String,Integer>,Integer,Integer>(){
			private static final long serialVersionUID = 1L;
			@Override
		...
		ArrayList<Tuple2<Integer, Integer>> blockSizesAndFreq = new ArrayList<Tuple2<Integer, Integer>>(blocksFreq.collect());
		
		
		/* blockSizes: 
		 * sorted array of (blockSize,Frequency) tuples in ascending order 
		 */
		Collections.sort(blockSizesAndFreq,new Comparator<Tuple2<Integer, Integer>>(){
			@Override
		...
		/*
		 * statistics:
		 * array of pairs (blockSize, CC) for every blockSize
		 */
		...
		for(int i = 0; i < blockSizesAndFreq.size(); i++){
			...
			Tuple2<Integer,Double> st = new Tuple2<Integer,Double>(blockSize.intValue(),CC);
			statistics.add(st);
		...
		/*
		 * find minimum difference for every adjacent pair i,i-1 
		 * the minimum difference represents the optimal blockSize 
		 */
